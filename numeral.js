...
(function (global, factory) {
    ...
    ************************************/
    ...
    (function () {
        ...
        // Also exclude crypto symbols BTC, ETH, SOL, DOGE
        unformatRegex = '(' + unformatRegex.replace('B', 'B(?!PS|TC)') + ')';

        numeral.register('format', 'bytes', {
            regexps: {
                format: /([0\s]i?b)/,
                unformat: new RegExp(unformatRegex, 'i')
            },
            format: function (value, format, roundingFunction) {
                var output,
    ...
    (function () {
        ...
        numeral.register('format', 'crypto', {
            regexps: {
                ...
                unformat: /(-?\d+\.?\d*)\s+(btc|eth|sol|doge)(?:\s|$)/i  // More specific: requires space before crypto AND word boundary
            },
            format: function (value, format, roundingFunction) {
            ...
            unformat: function (string) {
                var parts = string.match(/(-?\d+\.?\d*)\s+(btc|eth|sol|doge)(?:\s|$)/i);  // Updated to match the regex above
